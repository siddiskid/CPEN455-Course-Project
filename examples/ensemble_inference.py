#!/usr/bin/env python3
"""
Ensemble inference for spam classification.

Usage:
    uv run -m examples.ensemble_inference --checkpoints ckpts/final_model_seed0.ckpt ckpts/final_model_seed42.ckpt
"""
...
def ensemble_inference(args, models, tokenizer, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Ensemble inference"):
            ...
            for i, idx in enumerate(indices):
                all_predictions[idx] = {
                    "prob_ham": avg_probs[i, 0].item(),
                    "prob_spam": avg_probs[i, 1].item(),
                }
...
if __name__ == "__main__":
    ...
    predictions = ensemble_inference(args, models, tokenizer, test_dataloader, device)
